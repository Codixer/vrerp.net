...
export const PullstateCore = createPullstateCore({
    variantStore, siteStore, profileStore, calendarStore, assetStore, mediaStore, datesStore,
});
...
export const complimentTags = ['Nice profile', 'Good phantom sense', 'Cute avatar', 'Great cuddles', 
    'Nice voice', 'Cute noises', 'Nice Moves', 'Good vibes', 'Great conversation'];
...
export const getBucketFilename = (file, size) => {
    const fn = (size === 'original') ? `original/${ file.filename }` : `${ size }.${ file.extension || 'png' }`;
    let directory = (!file.category || file.category === 'profile') && `usermedia/${ file.userId }`;
    directory = directory || (file.category === 'fantasy') && `fantasy/${ file.parentId }`;
    directory = directory || (file.category === 'verification') && `verification/${ file.parentId }`;
    directory = directory || (file.category === 'asset') && `asset/${ file.parentId }`;
    directory = directory || (file.category === 'assetimage') && `assetimage/${ file.parentId }`;
...
export const REPORT_TAGS = {
    ...
    redflag: [
        { name: 'consent violation', value: 'Consent violation: doing things you haven\'t agreed upon, and didn\'t liked; eg: going on after safewords, or photography without consent, etc' },
        { name: 'lying on profile', value: 'Lying on profile (eg pretending to be fullbody, on desktop)' },
        { name: 'dm policy violation', value: 'Not respecting DM policy, or spam inviting/harassment' },
        { name: 'red flag violation', value: 'Something else' },
    ],
    yellowdot: [
        { name: 'bad roleplay', value: 'Bad roleplay / foreplay' },
        { name: 'lack of effort', value: 'Lack of effort / communication / reciprocation during ERP' },
...
export const allRoles = ['onboarded', 'verified', 'admin', 'mod', 'suspended', 'banned', 'dates', 'featured', 'events', 'hidden'];
