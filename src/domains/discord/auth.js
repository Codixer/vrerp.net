...
async function getAccessToken(code, api) {
    ...
    try {
        ...
        const auth = `${ _.get(d, 'data.token_type') } ${ _.get(d, 'data.access_token') }`;
        return auth;
    } catch (e) {
        console.error(e);
        return null;
...
async function getDiscordProfile(req, api) {
    ...
    if ((['development', 'testing'].includes(process.env.MODE)) && (code === 'testingcode')) {
...
router.get('/api/discord/link', async (req, res, next) => {
    ...
    const loginUser = await User.findOne({ _id: req.session.userId }).lean().exec();
    const testUser = await User.findOne({ discordId: discordProfile.id }).lean().exec();
    if (testUser) {
        console.error('discord user already', req.user, testUser);
        res.send('There is a user with this discord profile already -we need to delete it before you can link it. DM Codixer#2936 to troubleshoot');
        return;
    }
    const discord = `${ discordProfile.username }#${ discordProfile.discriminator }`;
    await User.findOneAndUpdate({ _id: req.session.userId }, { discord, discordId: discordProfile.id });
    const previousStatus = await Status.findOne({ discordId: discordProfile.id }).lean().exec();
    const updatedStatus = { discord, discordId: discordProfile.id };
    if (previousStatus) {
        if (previousStatus.userId && previousStatus.userId.toString() !== req.session.userId.toString()) {
...
router.get('/api/discord/auth', async (req, res, next) => {
    ...
    if (req.cookies['invite-code']) {
        ...
        await applyInviteCode(user._id, user.discordId, discord, req.cookies['invite-code']);
