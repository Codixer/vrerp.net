...
export async function uploadRoles() {
    ...
    const discordRoles = clientGuild.roles.cache.sort((a, b) => (b.position - a.position));
    ...
    Promise.allSettled(discordMissingRoles.map(async (tag) => {
        ...
        if (role.size > 0) {
            ...
            const k = await DiscordRole.create({ _id: generateSnowflake(), tag, roleId: newrole.id, guildId });
...
async function deleteServerRoles() {
    const clientGuild = client.guilds.cache.get(guildId);
    const roles = await DiscordRole.find({ guildId }).lean().exec();
    const res = await Promise.allSettled(roles.map(async (role) => {
        return Promise.all([
...
export async function syncMemberRoles(userId) {
    ...
    let allProfileTags = [];
    if (verified) {
        allProfileTags = Object.keys(profileSchema).reduce((arr, item) => arr.concat(profile[item] ? profile[item] : []), []);
    ...
    const existingTags = member.roles.cache.sort((a, b) => (b.position - a.position)).map(r => r.name);
...
async function syncAllMemberRoles() {
    const clientGuild = client.guilds.cache.get(guildId);
    const members = clientGuild.members.cache.map(m => (m.user && m.user.id) ? m.user.id : null).filter(id => id);
    const limit = pLimit(5);
    await Promise.all(members.map((discordId) => limit(async () => {
...
async function syncMemberStatus() {
    ...
    const members = clientGuild.members.cache.map(m => (m.user && m.user.id) ? m.user.id : null).filter(id => id);
    const limit = pLimit(5);
    await Promise.all(clientGuild.members.cache.map((member) => limit(async () => {
    ...
    (async () => {
        ...
        client.once('ready', async () => {
            ...
            if (process.argv.length < 3) {
                console.log('usage: roles.js [refresh | members | status | reset | member]');
                process.exit(0);
            }
