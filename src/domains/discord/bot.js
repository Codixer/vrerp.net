...
export const client = new Client({ intents: [Intents.FLAGS.GUILDS, Intents.FLAGS.GUILD_PRESENCES, Intents.FLAGS.GUILD_MEMBERS] });
...
export async function sendMessage(discordId, message) {
    ...
    if (user) {
        console.log('sending message to user', discordId.toString(), user.username, message);
        try {
            await user.send(message);
        } catch (err) {
            console.warn('failed to send message to user ',discordId.toString(), user.username);
        }
    } else {
        console.warn('failed to find user ',discordId.toString(), user.username, message);
    }
}
...
export const discordLog = async (message) => sendChannel(mainGuildId, mainLogsChannel, message);
...
async function inviteNotifications(userId, parameters) {
    ...
    if ((!userProfile || !(userProfile.discordNotifications & NOTIFICATION_INVITE))) {
        return false;
    }
    const otherUserProfile = await getProfile(parameters.sourceProfileId);
    await sendMessage(user.discordId, `${ otherUserProfile.username } wants to be your lewdie 💕 ${ process.env.SERVER_URL }${ otherUserProfile.url }`);
}
...
async function matchNotifications(userId, parameters) {
    ...
    const userProfile = await Profile.findOne({ _id: user.profileId });
    if ((!userProfile || !(userProfile.discordNotifications & NOTIFICATION_MATCH))) {
        return false;
    }
    const otherUserProfile = await getProfile(parameters.sourceProfileId);
    await sendMessage(user.discordId, `Matched with ${ otherUserProfile.username } 💖 ${ process.env.SERVER_URL }${ otherUserProfile.url }`);
}
...
async function approveVerification(userId, parameters) {
    ...
    const verification = await Verification.findOne({ discordId }).sort({ updatedAt: -1 }).lean().exec();
    ...
    await Promise.allSettled(enableDiscords.map(async (d) => {
...
async function suspendedMember(userId, parameters) {
    ...
    await sendMessage(user.discordId, `Your vrerp.net account has been suspended: ${ parameters.message }. You can re-activate it on the site: ${ process.env.SERVER_URL } `);
}

async function submittedVerification(userId, parameters) {
    const { discordId } = parameters;
    const admins = await User.find({roles: { $in: ['mod', 'admin'] } }).lean().exec();
    const verification = await Verification.findOne({ discordId }).sort({ updatedAt: -1 }).lean().exec();
    console.log('admins', admins);
    await Promise.all(admins.map((user) => sendMessage(user.discordId, `New ID verification submitted: ${ process.env.SERVER_URL }moderator/verification    ${ verification ? verification.discord : '' }`)));
}

client.once('ready', async () => {
    console.log('discord bot ready');
    Events.listen(EventTypes.INVITED, Events.ListenerType.GLOBAL, inviteNotifications);
    Events.listen(EventTypes.MATCHED, Events.ListenerType.GLOBAL, matchNotifications);
    Events.listen(EventTypes.VERIFICATION_APPROVED, Events.ListenerType.GLOBAL, approveVerification);
    Events.listen(EventTypes.VERIFICATION_DENIED, Events.ListenerType.GLOBAL, deniedVerification);
    Events.listen(EventTypes.VERIFICATION_SUBMITTED, Events.ListenerType.GLOBAL, submittedVerification);
    Events.listen(EventTypes.USER_SUSPENDED, Events.ListenerType.GLOBAL, suspendedMember);
});

client.on('presenceUpdate', async (oldMember, newMember) => {
    console.log('member', _.get(newMember, 'member.user.username'), newMember.status);
...
client.on('guildMemberAdd', async (member) => {
    console.log(`new member joined ${ member.guild.name }: ${ _.get(member, 'user.username') }`);
    const guild = await Guild.findOne({ discordId: member.guild.id }).lean().exec();
    console.log('guild', guild);
    ...
    await updateUserStatus({ discordId }, { discordStatus: member.status }, { $addToSet: { guilds: member.guild.id } });
    // await attemptAutoVerification(discordId, discord);
    let verification = await Verification.findOne({ discordId }).sort({ updatedAt: -1 }).lean().exec();
    if (verification) {
    ...
    await sendMessage(discordId, `Hi-hi, welcome to horny 🤗 We're a site dedicated to erp matchmaking ( https://vrerp.net/ ), and a discord community for flirting, meeting new lewdies💕, events, and more!
...
client.on('guildMemberRemove', async (member) => {
    console.log(`member left ${ member.guild.name }: ${ _.get(member, 'user.username') }`);
    const guild = await Guild.findOne({ discordId: member.guild.id }).lean().exec();
    if ((!guild) || (!guild.ageVerification) || (!_.get(member, 'user.id'))) {
        return;
    }
    const discordId = _.get(member, 'user.id');
    await updateUserStatus({ discordId }, { discordStatus: 'offline' }, { $pull: { guilds: member.guild.id } });
});
