...
export const getPage = async (url) => {
  ...
  const profile = await Profile.findOne({ url: username.toLowerCase() })
    .lean()
    .exec();
  if (profile) {
    const image = profile.avatar
      ? await UserFile.findOne({ _id: profile.avatar }).lean().exec()
      : null;
  ...
  if (page) {
    return {
      type: "page",
      id: page._id,
      ..._.pick(page, ["description", "content", "title", "url", "subtitle"]),
    };
  }
...
export const getGuides = async () =>
  _.keyBy(
    (await Page.find({}).lean().exec()).map((p) => {
      return {
        type: "page",
        ..._.pick(p, ["url", "title", "subtitle", "description"]),
      };
    }),
    "url"
  );
...
export const refreshPage = async (page) => {
  const result = await axios.get(
    `https://api.telegra.ph/getPage/${page.telegraphPage}?access_token=${process.env.TELEGRAPH_TOKEN}&return_content=true`
  );
