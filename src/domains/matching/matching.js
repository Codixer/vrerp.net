...
export const getInvitePending = async(profileId) => {
    const c = await Match.find({ category: 'invites', list: profileId }).lean().exec();
    return c.map((row) => row.profileId.toString());
}

export const addMatchDb = async(profileId, category, matchProfileId) => {
    const res = await Match.findOneAndUpdate({ profileId, category }, { $addToSet: { list: matchProfileId } }, { new: true, upsert: true });
    return res.list.map((item) => item.toString());
};

export const removeMatchDb = async(profileId, category, matchProfileId) => {
    const res = await Match.findOneAndUpdate({ profileId, category }, { $pull: { list: matchProfileId } }, { new: true, upsert: true });
    return res.list.map((item) => item.toString());
}
...
router.get('/api/match/next', async(req, res) => {
    ...
    ).sort({ available: -1, lastActivity: -1 }).limit(1).lean().exec();
    if (next.length === 0) {
...
router.post('/api/match/set-match', async(req, res) => {
    ...
    if (req.body.match === 'love') {
        ...
        if (invites.includes(otherPerson)) {
            ...
            Events.emit(EventTypes.MATCHED, otherPersonProfile.userId, { sourceProfileId: req.user.profileId });
        } else {
            await addMatchDb(thisPerson, 'loves', otherPerson);
            await addMatchDb(otherPerson, 'invites', thisPerson);
            // eliminate invite-spamming
            const prevInvite = await UserEvent.findOne({ type: EventTypes.INVITED, userId: otherPersonProfile.userId, 'parameters.sourceProfileId': req.user.profileId }).lean().exec();
            if (!prevInvite) {
                Events.emit(EventTypes.INVITED, otherPersonProfile.userId, { sourceProfileId: req.user.profileId });
            }
        }
        ...
        if (existingMatches.includes(otherPerson)) {
            const otherPersonProfile = await getProfile(req.body.profileId);
            Events.emit(EventTypes.UNMATCHED, otherPersonProfile.userId, { sourceProfileId: req.user.profileId });
        }
        await removeMatchDb(thisPerson, 'invites', otherPerson);
...
router.delete('/api/match/passes', async(req, res) => {
    ...
    await Match.deleteOne({ profileId: req.user.profileId, category: 'passes' }).exec();
    return res.send({ status: 'ok', data: await getMatchData(req.user.profileId) });
});
