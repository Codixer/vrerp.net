...
export async function updateInternalProfile(profile) {
  ...
  if (profile.compliments) {
    let allIds = [];
    Object.keys(profile.compliments)
      .filter((k) => !["Nice profile", "Cute avatar"].includes(k))
      .map((k) => {
  ...
  if (profile.gender && profile.orientation) {
    ...
    const gender =
      typeof profile.gender === "string" ? [profile.gender] : profile.gender;
    ...
    const orientation =
      typeof profile.orientation === "string"
        ? [profile.orientation]
        : profile.orientation;
  ...
  // console.log(update);
  await ProfileInternal.findOneAndUpdate({ _id: profile._id }, update, {
    new: true,
    upsert: true,
  });
}
...
async function prefillInternals() {
  ...
  const profiles = await Profile.find({ _id: { $in: missingIds } })
    .lean()
    .exec();
  while (profiles.length > 0) {
    console.log(profiles.length);
    await Promise.all(
      profiles.splice(0, 100).map((p) => updateInternalProfile(p))
    );
  }
