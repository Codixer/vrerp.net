...
import { deleteUserStatus, getUserStatus, updateUserStatus } from '../status/status.js';
...
export const getSessionParser = () => {
    if (!sessionParser) {
        sessionParser = session({
            ...
            store: MongoStore.create({ client: mongoose.connection.client, collectionName: 'sessions', stringify: false }),
...
async function basicData(req, res, next) {
    if (req.session.userId) {
        ...
        req.user = _.pick(user, ['_id', 'profileId', 'roles', 'discordId', 'adminMessage', 'inviteCode', 'email']);
        req.user.id = req.user._id;
        req.user.reported = publicReport(await Report.findOne({ accusedId: req.user.id, status: 'userNotification' }).lean().exec());
        req.userProfile = await Profile.findOneAndUpdate({ _id: user.profileId }, { $set: { lastActivity: new Date() }}, { new: true });
...
export async function createUser(userInfo = {}) {
    ...
    const profile = new Profile({ _id: generateSnowflake(), userId, discord: userInfo.discord });
    let baseUsername = userInfo.discord ? userInfo.discord.split('#')[0] : userInfo.email.split('@')[0];
    profile.username = baseUsername.replace(/[ #/\\._-]/g, '-').replace(/[#!?]/g, '');
    for (let i = 0; i < 10; i++) {
...
export async function getUserFromString(s) {
    let profile = await Profile.findOne({ $or: [{ url: s }, { username: s }, { discord: s }] }).lean().exec();
    if (profile) {
...
export async function banUser(userId, adminMessage) {
    const profile = await Profile.findOneAndUpdate({ userId }, { profileVisibility: 'banned' }, { new: true }).exec();
    await User.findOneAndUpdate({ _id: userId }, { $addToSet: { roles: 'banned', adminMessage: adminMessage }}).exec();
...
router.post('/api/users/activate', async (req, res) => {
    if (!req.user || !req.user.roles || !req.user._id || !req.user.roles.includes('verified')) {
        return res.status(400).send({ error: 'please log in' });
    }
    await User.findOneAndUpdate({ _id: req.user._id }, { $addToSet: { roles: 'onboarded' }});
...
router.post('/api/users/deactivate', async (req, res) => {
    ...
    const profile = await Profile.findOneAndUpdate({ _id: req.user.profileId }, { profileVisibility: 'suspended' }, { new: true }).exec();
    await User.findOneAndUpdate({ profileId: req.user.profileId }, { $addToSet: { roles: 'suspended' }, $set: { adminMessage: 'Suspended by your request' } }).exec();
    req.sessionStore.generate(req);
    delete req.user;
...
router.post('/api/users/reactivate', async (req, res) => {
    ...
    const profile = await Profile.findOneAndUpdate({ _id: req.user.profileId }, { profileVisibility: 'members' }, { new: true }).exec();
    await User.findOneAndUpdate({ profileId: req.user.profileId }, { $pull: { roles: 'suspended' }, $unset: { adminMessage: '' } }).exec();
    return res.send({ status: 'ok' });
});
...
router.post('/api/users/status', async (req, res) => {
    ...
    if (!['horny', 'available', 'online', 'unavailable'].includes(req.body.status)) {
        return res.status(400).send({ error: 'invalid status' });
    }
    await updateUserStatus({ userId: req.user._id }, { hornyStatus: req.body.status });
    res.send({ status: 'ok', data: await hydrateUser(req) });
});
...
export const hydrateUser = async(req) => {
    ...
    if (req.user) {
        ...
        if (fullAccess(req.user) && !req.user.inviteCode) {
            data.inviteCode = req.user.inviteCode = await generateInviteCode(req.user._id);
        }
        data = { ...data, ...await getUserStatus({ userId: req.user._id }), messages: await getFrontpageMessages(req) };
    }
    return { user: data };
};
