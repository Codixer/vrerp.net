...
export const getPublicAsset = (data) => { return { ... _.omit(data, '_id', '__v', 'description'), id: data['_id'], price: data.price ? data.price.toString() : null } };
...
export const getAssets = async(req, ids) => {
    ...
    const assetData = await Asset.find({ _id: { $in: ids } }).lean().exec();
    const assets = assetData.map((item) => getPublicAsset(item));
    const files = await getFilesByIds(assets.map((item) => item.image).filter(item => item));
    return { assets: _.keyBy(assets, 'id'), files };
};

export const getAssetList = async (req, query, lastId, status = 'published') => {
    const filter = { };
    if (status) {
        filter.status = status;
    }
    const limit = (req && req.query && req.query.limit) ? parseInt(req.query.limit) : 32;
    if (lastId) {
        filter._id = { $lt: lastId };
    }
    let queryparts = query ? query.toLowerCase().split('-').filter((t) => !['vrchat', 'erp', 'assets'].includes(t)) : [];
    ...
    const assetData = await Asset.find(filter).sort({ _id: -1 }).limit(limit).lean().exec();
...
router.get('/api/assets', async (req, res) => {
    res.send({ status: 'ok', data: await getAssetList(req, req.query.query, req.query.lastId) });
});
...
router.post('/api/assets', async (req, res) => {
    ...
    discordLog(`New asset posted ${ req.body.url }\n${ process.env.SERVER_URL }admin/assets`)
    res.send({ status: 'ok' });
});
...
router.post('/api/assets/:id', async (req, res) => {
    ...
    if (req.body.status && ['draft', 'published', 'deleted' ].includes(req.body.status)) {
...
router.post('/api/assets/:id/click', async (req, res) => {
    ...
    Events.emit(EventTypes.ASSET_CLICKED, req.user ? req.user.id : null, { assetId: req.params.id });
    res.send({ status: 'ok' });
});
