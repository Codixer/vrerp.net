...
import { getProfileData, getProfileList, profileSchema } from '../users/profiles.js';
...
const getSchemaValues = () => Object.keys(profileSchema).reduce((arr, val) => {
...
router.get('/api/search', async (req, res, next) => {
    ...
    if (req.query.q) {
        ...
        const qs = req.query.q.split(' ').filter(q => !['female'].includes(q.toLowerCase()));
        qs.forEach((q) => {
            const qfilters = [];
    ...
    if (req.query.sort && req.query.sort === 'availability') {
        sortingPipeline = [
            { $addFields: { statusNumeric: { $ifNull: [ { $arrayElemAt: ['$status.order', 0] }, 4 ] } } },
            { $addFields: { statusOrder: { $cond: { if: { $eq: ['$statusNumeric', -1 ] }, then: 4, else: '$statusNumeric'  } } } },
