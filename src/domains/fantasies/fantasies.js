...
export const getPublicFantasy = (data) => {
  return { ..._.omit(data, "_id", "__v"), id: data["_id"] };
};
...
export const getFantasyLikes = async (fantasyList) => {
  const likeData = await FantasyProfile.aggregate([
    { $group: { _id: "$fantasyId", profiles: { $addToSet: "$profileId" } } },
    {
      $match: {
        _id: {
          $in: fantasyList.map((id) => mongo.Long.fromString(id.toString())),
        },
      },
    },
  ]);
  const profileList = Array.from(
    new Set(likeData.reduce((arr, val) => arr.concat(val.profiles), []))
  );
  const profileUrls = await Profile.find({
    ...
    .select(["_id", "url", "username"])
    .lean()
    .exec();
  const profileMap = profileUrls.reduce((arr, val) => {
    arr[val._id] = val.url;
    return arr;
  }, {});
  const fantasyLikes = likeData.reduce((arr, val) => {
    arr[val._id] = val.profiles
      .map((p) => profileMap[p.toString()])
      .filter((p) => p);
    return arr;
  }, {});
  fantasyList.forEach(
    (item) => (fantasyLikes[item] = fantasyLikes[item] || [])
  );
  return fantasyLikes;
};
...
export const getFantasyAuthors = async (fantasies) => {
  ...
  return fantasies.map((item) => {
    item.author = profileMap[item.authorId];
    return item;
  });
};
...
export const getFantasyList = async (lastId) => {
  ...
  let data = await Fantasy.find(filter)
    .sort({ createdAt: -1 })
    .limit(10)
    .lean()
    .exec();
  ...
  const files = await getFantasyFiles(
    data.map((d) => d.image).filter((d) => d)
  );
...
export const getFantasies = async (ids) => {
  ...
  let fantasies = await Fantasy.find({ _id: { $in: ids } })
    .lean()
    .exec();
...
fantasyRouter.post("/api/fantasies", async (req, res) => {
  ...
  let fantasy = await Fantasy.create({
...
fantasyRouter.get("/api/fantasies/:id", async (req, res) => {
  if (!req.params.id) {
    return res
      .status(400)
      .send({ error: "logged in user, and fantasy id required" });
  }
  ...
  if (
    fantasy.status === "draft" &&
    (!req.session.userId ||
      fantasy.authorId.toString() !== req.user.profileId.toString())
  ) {
...
fantasyRouter.post("/api/fantasies/:id", async (req, res) => {
  ...
  fantasy = await Fantasy.findOneAndUpdate(
    { _id: req.params.id },
    { $set: updates },
    { new: true }
  )
...
fantasyRouter.post("/api/fantasy-profile/:id/:type", async (req, res) => {
  ...
  const doc = {
    fantasyId: req.params.id,
    profileId: req.user.profileId,
    type: req.params.type,
  };
  await FantasyProfile.findOneAndUpdate(doc, doc, { new: true, upsert: true })
    .lean()
    .exec();
  res.send({
    status: "ok",
    data: {
      profile: await getProfileData(req, await getProfile(req.user.profileId)),
    },
  });
});
...
fantasyRouter.delete("/api/fantasy-profile/:id/:type", async (req, res) => {
  ...
  await FantasyProfile.deleteOne({
    fantasyId: req.params.id,
    profileId: req.user.profileId,
    type: req.params.type,
  }).exec();
...
export const deleteFantasyById = async (id) => {
  ...
  const fantasy = await Fantasy.findOne({ _id: mongo.Long.fromString(id) })
    .lean()
    .exec();
