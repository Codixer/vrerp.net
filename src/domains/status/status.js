...
async function createUserStatus(where) {
    ...
    const newStatus = await Status.findOneAndUpdate(where, {
...
export async function getUserStatus(where) {
    ...
    const guildPresence = status.guilds ? status.guilds.map(g => g.toString()) : [];
    return { status: status.hornyStatus, discordPresence: (status.guilds && guildPresence.includes(mainGuildId)) };
}
...
function calculateStatus(status) {
    ...
    if ((status.hornyStatus === 'online') && (status.webStatus === 'offline') && (status.discordStatus === 'idle')) {
        return 'idle';
    }
...
export async function addWebClientToUserStatus(where, clientId) {
    ...
    if (oldStatus && oldStatus.webClients && oldStatus.webClients.includes(clientId)) {
        return oldStatus.webClients.length;
    }
    await updateUserStatus(where, { webStatus: 'online', $addToSet: { webClients: clientId }});
}

export async function removeWebClientFromUserStatus(where, clientId) {
    await updateUserStatus(where, { webStatus: 'offline', $pull: { webClients: clientId }});
}
...
export async function updateUserStatus(where, update, funcUpdate = {}) {
    ...
    if (oldStatus && oldStatus.displayStatus !== displayStatus) {
        Events.emit(EventTypes.PROFILE_STATUS_CHANGED, res.userId, { displayStatus, order, hornyStatus: res.hornyStatus });
    }
}
...
export async function expireHornyStatus() {
    ...
    const expiredStatuses = await Status.find({
        hornyStatus: 'horny', $or: [ { hornyExpires: { $exists: false } }, { hornyExpires: { $lt: new Date() } } ] 
    }).lean().exec();
    await Promise.all(expiredStatuses.map((status) => updateUserStatus({ userId: status.userId }, { hornyStatus: 'available' })));
    return true;
}
