...
export async function getMediaList(filter) {
    const mediaData = await UserFile.find(filter).sort({ _id: -1 }).limit(32).lean().exec();
    const userData = await getUserToProfileMapping(mediaData.filter(d => d.userId).map(d => d.userId.toString()));
    const files = _.keyBy(mediaData.map(d => publicFile(d)).map(d => {
