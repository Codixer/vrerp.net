...
export async function setVerificationStatus(
  ...
  let verification = await Verification.findOne(
    verificationQuery(discordId, userId)
  )
    .sort({ updatedAt: -1 })
    .lean()
    .exec();
  if (!verification || verification.status === "failed") {
    ...
    verification = await Verification.findOneAndUpdate(
      ...
      { status, adminId, rejectionReason },
      { new: true }
    );
  }
  if (status === "verified") {
    const verifiedUser = await User.findOneAndUpdate(
      userId ? { _id: userId } : { discordId },
      { $addToSet: { roles: "verified" } },
      { new: true }
    ).exec();
    if (verifiedUser) {
      const existingProfile = await Profile.findOne({
        _id: verifiedUser.profileId,
      })
        .lean()
        .exec();
      if (
    ...
    Events.emit(EventTypes.VERIFICATION_APPROVED, verifiedUserId, {
    ...
    Events.emit(EventTypes.VERIFICATION_DENIED, userId, {
      discordId: verification.discordId,
...
router.get("/api/verification", async (req, res) => {
  ...
  // }
  let verification = await Verification.findOne(
    verificationQuery(req.user.discordId, req.user._id)
  )
    .sort({ updatedAt: -1 })
    .lean()
    .exec();
...
router.post("/api/verification", async (req, res) => {
  ...
  if (
    verification &&
    ["draft", "banned", "verified"].includes(verification.status)
  ) {
  ...
  res.send({
    status: "ok",
    data: { verification: publicObject(verification.toObject()) },
  });
});
...
router.get("/api/verification/:id", async (req, res) => {
  ...
  const verification = await Verification.findOne({ _id: req.params.id })
    .lean()
    .exec();
  if (!verification) {
  ...
  res.send({
    status: "ok",
    data: { verification: publicObject(verification), files },
  });
});
...
router.post("/api/verification/:id", async (req, res) => {
  ...
  if (req.body.status === "submit") {
    ...
    Events.emit(EventTypes.VERIFICATION_SUBMITTED, verification.userId, {
      discordId: verification.discordId,
    });
  }
  verification = await Verification.findOneAndUpdate(
...
if (isMainEntry(import.meta.url)) {
  (async () => {
    ...
    const writeops = items.map((discordId) => {
      return {
        updateOne: {
          filter: { discordId },
          update: {
