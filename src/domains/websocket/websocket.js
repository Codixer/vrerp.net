...
import { addWebClientToUserStatus, removeWebClientFromUserStatus } from '../status/status.js';
...
async function addClient(client) {
    ...
    clientsByProfileId[client.profileId] = clientsByProfileId[client.profileId] || [];
    clientsByProfileId[client.profileId].push(client);
    if (clientsByUserId[client.userId].length === 1) {
        await addWebClientToUserStatus({ userId: client.userId }, process.env.WORKERID);
    }
}

async function removeClient(client) {
    console.log('removeclient', client.profileId);
    clientsByUserId[client.userId] = clientsByUserId[client.userId].filter((c) => c !== client);
    clientsByProfileId[client.profileId] = clientsByProfileId[client.profileId].filter((c) => c !== client);
    if (clientsByUserId[client.userId].length === 0) {
        await removeWebClientFromUserStatus({ userId: client.userId }, process.env.WORKERID);
    }
}
...
async function broadcastMessage(profileIds, message) {
    ...
    const allClients = profileIds.reduce((acc, val) => acc.concat(clientsByProfileId[val] || []), []);
    allClients.map((c) => {
...
async function userStatusChanged(userId, parameters) {
    ...
    await broadcastMessage(broadcastList, { type: 'status-update', data: { profileId: user.profileId, status: parameters.displayStatus, hornyStatus: parameters.hornyStatus } });
}
...
export async function initWebSocket(server) {
    ...
    Events.listen(EventTypes.PROFILE_STATUS_CHANGED, Events.ListenerType.INSTANCE, userStatusChanged);
}
