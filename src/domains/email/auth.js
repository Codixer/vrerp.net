...
router.post('/api/users', async (req, res) => {
    ...
    if (user.loginCodes.length > 5) {
        ...
        return res.status(400).send({ error: 'Exceeded quota, please create a support ticket on discord.' });
    }
    const code = Math.random().toString(36).substring(2).substring(0,8);
    await User.findOneAndUpdate({ _id: user._id }, { $addToSet: { loginCodes: code } }).exec();
    const emailData = <div><EmailWelcome server={ process.env.SERVER_URL } code={ code } /></div>;
...
router.get('/api/users/auth', async (req, res) => {
    ...
    await User.findOneAndUpdate({ _id: user._id }, { $pull: { loginCodes: req.query.code }  }).exec();
...
router.get('/testing/emails/:type', async (req, res) => {
    ...
    const profileList = await getProfileList(null, ['990820298175635457'], ['profile']);
    const profile = Object.values(profileList.profiles)[0];
    const avatar = profileList.files[profile.avatar];
    console.log(profile);
    const code = 'test';
    const Body = <div>
        { req.params.type === 'welcome' && <EmailWelcome server={ process.env.SERVER_URL } code={ code } /> }
        { req.params.type === 'match' && <EmailMatch server={ process.env.SERVER_URL } code={ code } profile={ profile } avatar={ avatar } /> }
        { req.params.type === 'invite' && <EmailInvite server={ process.env.SERVER_URL } code={ code } profile={ profile } avatar={ avatar } /> }
        { req.params.type === 'approved' && <EmailVerificationApproved server={ process.env.SERVER_URL } code={ code } /> }
        { req.params.type === 'denied' && <EmailVerificationDenied server={ process.env.SERVER_URL } code={ code } /> }
    </div>;
