...
export async function flagUser(userId, reportId, days) {
  ...
  expireDate = profile.flagsExpire
    ? new Date(Math.max(expireDate, profile.flagsExpire))
    : expireDate;
...
router.post("/api/reports", async (req, res, next) => {
  ...
  discordLog(
    `<@&1088201944604094529> New report made for \`${
      ...
      reportData.tags == null ? `No tags selected` : reportData.tags.toString()
