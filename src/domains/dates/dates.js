...
router.get('/api/dates/timezones', async (req, res, next) => {
    ...
    const dateProfile = await UserDateProfile.findOne({ userId: req.session.userId }).lean().exec();
...
async function updateAvailability(req) {
    ...
    if ((req.body.userTimezoneOffset) && (!isNaN(parseInt(req.body.userTimezoneOffset)))) {
        update.userTimezoneOffset = parseInt(req.body.userTimezoneOffset);
    }
    if (req.body.available) {
        update.available = req.body.available;
    }
    await UserDateProfile.findOneAndUpdate({ userId: req.session.userId }, update, { upsert: true, new: true } );
}
...
router.get('/api/dates/available/:url', async (req, res, next) => {
    ...
    const dateProfile = await UserDateProfile.findOne({ userId: profile.userId }).lean().exec();
    if (!dateProfile) {
...
const getDates = async(userId) => UserDateInstance.find({ $or: [{ invited: userId }, {inviter: userId }], startDate: { $gt: new Date() }}).sort({ createdAt: -1 }).lean().exec();
...
router.get('/api/dates', async (req, res, next) => {
    ...
    const allUserIds = _.uniq(_.flatMap(dates, (date) => [date.invited, date.inviter])).filter((id) => id.toString() !== req.user.id.toString());
    const allProfileIdMap = await Profile.find({ userId: { $in: allUserIds } }).select(['_id', 'userId']).lean().exec();
