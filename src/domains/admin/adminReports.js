...
router.get("/api/moderator/reports", async (req, res) => {
  let reports = await Report.find({ status: "adminReview" })
    .sort({ createdAt: 1 })
    .lean()
    .exec();
  const allUsers = _.uniq(
    reports
      .reduce((acc, val) => acc.concat([val.victimId, val.accusedId]), [])
      .filter((id) => id)
  );
  const profiles = _.keyBy(
    await Profile.find({ userId: { $in: allUsers } })
  ...
  reports = reports.map((r) => {
    return {
      ...r,
      id: r._id,
      accusedUrl: r.accusedId
        ? _.get(profiles, [r.accusedId.toString(), "url"])
        : null,
      victimUrl: r.victimId
        ? _.get(profiles, [r.victimId.toString(), "url"])
        : null,
    };
  });
...
router.post("/api/moderator/reports/:id", async (req, res) => {
  ...
  const adminDecisionDays =
    req.body && req.body.days ? parseInt(req.body.days) : 3;
  if (req.body.result === "none") {
    await Report.findOneAndUpdate(
      { _id: req.params.id },
      { status: "closed", adminId, adminDecision: "none" }
    );
  } else if (req.body.result === "warn") {
    await Report.findOneAndUpdate(
      { _id: req.params.id },
      { status: "userNotification", adminId, adminDecision: "warn" }
    );
    ...
    await Report.findOneAndUpdate(
      { _id: req.params.id },
      { status: "closed", adminId, adminDecision: "ban" }
    );
  }
  res.send({ status: "ok" });
});
