...
router.get(['/api/moderator/profiles', '/api/moderator/member-profiles'], async (req, res) => {
    console.log(req);
    let filter = { };
    if (req.query.status) {
    ...
    const allprofiles = (await Profile.find(filter).select({ _id: 1 }).sort({ createdAt: -1 }).limit(64).lean().exec());
    const ids = allprofiles.map((p) => p._id);
    res.send({ status: 'ok', data: { ...await getProfileList(req, ids, adminAllowance), list: ids }});
});

router.post('/api/moderator/profiles', async (req, res) => {
    let existingProfile = await Profile.findOne({ username: req.body.username }).lean().exec();
    if (existingProfile) {
        return res.status(400).send({ error: 'Profile already exists' });
    }
    const profile = new Profile({ _id: generateSnowflake(), userId: req.user._id, username: req.body.username });
    await profile.save();
    res.send({ status: 'ok', data: { profileId: profile._id } });
});

router.get('/api/admin/users', async (req, res) => {
    const users = (await User.find({}).lean().exec()).map((user) => _.pick(user, ['_id', 'email', 'username', 'profileId']))
    res.send({ status: 'ok', data: { users } });
});

router.post('/api/moderator/moderate/:profileId/:moderate', async (req, res) => {
    if (req.params.moderate === 'banned') {
        ...
        profile = await Profile.findOne({ _id: req.params.profileId }).lean().exec();
        return res.send({ status: 'ok', data: { profile } });
    } else if (req.params.moderate === 'suspended') {
        const profile = await Profile.findOneAndUpdate({ _id: req.params.profileId }, { profileVisibility: req.params.moderate }, { new: true }).exec();
        await User.findOneAndUpdate({ profileId: req.params.profileId }, { $addToSet: { roles: req.params.moderate, adminMessage: req.body.adminMessage }}).exec();
        Events.emit(EventTypes.USER_SUSPENDED, profile.userId, { message: req.body.adminMessage });
        return res.send({ status: 'ok', data: { profile } });
        ...
        const profile = await Profile.findOne({ _id: req.params.profileId }).lean().exec();
        const user = await User.findOneAndUpdate({ _id: profile.userId }, { $pull: { roles: { $in: ['onboarded', 'verified'] }}}, { new: true }).exec();
        await Verification.deleteMany({ discordId: user.discordId }).exec();
        await Profile.findOneAndUpdate({ _id: req.params.profileId }, { profileVisibility: 'hidden' }).exec();
        await syncMemberRoles(profile.userId);
        return res.send({ status: 'ok', data: await getProfileList(req, [ req.params.profileId ]) });
    }
...
router.post('/api/admin/roles/:profileId', async (req, res) => {
    ...
    const profile = await Profile.findOne({ _id: req.params.profileId }).lean().exec();
    const user = await User.findOneAndUpdate({ _id: profile.userId }, { roles: req.body.newRoles }, { new: true }).lean().exec();
...
router.get('/api/moderator/search', async (req, res) => {
    ...
    console.log(filter);
    const profiles = await Profile.find({ $or: [{ _id: req.query.q }, { username: filter },{ discord: filter }, { vrchat: filter }, { url: filter }] }).select(['_id']).sort({ updatedAt: -1 }).lean().exec();
    ...
    res.send({ status: 'ok', data: { ...await getProfileList(req, list, adminAllowance), list }});
});
