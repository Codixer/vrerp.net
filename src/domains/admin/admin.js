...
import { getProfileList, getSchema, getUserToProfileMapping } from '../users/profiles.js';
...
router.post('/api/admin/calendartags', async (req, res) => {
    await CalendarEventTag.create({ _id: generateSnowflake(), name: req.body.name });
...
router.get('/api/admin/matches', async(req, res) => {
    ...
    matches = matches.filter((m) => m.user.length > 0).map((m) => { 
        return { source: m.parameters.sourceProfileId, target: m.user[0].profileId, matchDate: m.createdAt }
    });
    const ids = matches
        .reduce((acc, val) => acc.concat([val.source, val.target ]), [])
        .filter((v, i, a) => a.indexOf(v) === i);
    res.send({ status: 'ok', data: { ... await getProfileList(req, ids, ['profile', 'status', 'discord', 'fantasies']), matches }});
});
...
router.get('/api/admin/assets', async (req, res) => {
    res.send({ status: 'ok', data: await getAssetList(req, req.query.query, req.query.lastId, null) });
})
