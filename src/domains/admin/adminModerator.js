...
router.use('/api/moderator/*', async(req, res, next) => {
    if (!req.user || !req.user.roles || !(req.user.roles.includes('admin') || req.user.roles.includes('mod'))) {
...
async function addVerificationInfo(verifications) {
    ...
    const users = await User.find({ $or: [{ discordId: { $in: discordIds } }, { _id: { $in: userIds } }] }).lean().exec();
    const usersById = _.keyBy(users, '_id');
    const profileIds = users.map((u) => u.profileId);
    const allProfiles = await Profile.find({ _id: { $in: profileIds } }).lean().exec();
...
router.get('/api/moderator/verifications-audit-log', async(req, res) => {
    let verifications = (await Verification.find({ status: { $nin: [ 'pending', 'draft' ] } }).sort({ updatedAt: -1 }).limit(64).lean().exec()).map((d) => publicObject(d));
    res.send({ status: 'ok', data: await addVerificationInfo(verifications) });
});

router.get('/api/moderator/verifications', async(req, res) => {
    let verifications = (await Verification.find({ status: 'pending' }).sort({ updatedAt: 1 }).lean().exec()).map((d) => publicObject(d));
    res.send({ status: 'ok', data: await addVerificationInfo(verifications) });
});
...
router.post('/api/moderator/verifications/:id', async(req,res) => {
    ...
    verification = setVerificationStatus(verification.discordId, verification.discord, verification.userId, req.body.status, req.user.id, req.body.rejectionReason);
    res.send({ status: 'ok', data: { verification: publicObject(verification) } });
});

router.get('/api/moderator/mood', async (req, res) => {
    let moods = await UserEvent.find({ type: 'message-replied' }).sort({ updatedAt: -1 }).lean().exec();
    const userIds = Array.from(new Set(moods.map(m => m.userId)));
    let users = await Profile.find({ userId: { $in: userIds } }).select(['userId', 'url', 'username']).lean().exec();
    users = _.keyBy(users.map(u => { return { ..._.omit(u,'_id'), id: u.userId }}), 'id');
    moods = moods.map((m) => { m.group = `${ m.userId.toString() }-${ m.updatedAt.toISOString().substr(0,10) }`; return m; });
...
router.get('/api/moderator/images', async (req, res) => {
    ...
    let files = await UserFile.find(filter).sort({ createdAt: -1 }).limit(32).lean().exec();
    const userData = await getUserToProfileMapping(files.filter(d => d.userId).map(d => d.userId.toString()));
    const list = files.map((f) => f._id);
    files = _.keyBy(files.map(d => publicFile(d)).map((d) => { 
        return { ...d, ...(d.userId ? _.pick(userData[d.userId], ['username', 'url']) : { }) };
    }), 'id');
