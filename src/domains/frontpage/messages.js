...
const frontpageMessages = [
    {
        ...
        minDays: 7,
        frequency: 14,
        message: ['Hi-hi 🤗 thank you for being with us 💕',
    ...
    },
    {
        ...
        message: [
            'You\'re super cute 💕', 
            'Kitten would love to feature a few selected super cute peeps on the frontpage, and on the site.',
...
export const getFrontpageMessages = async(req) => {
    const replies = await UserEvent.find({ type: EventTypes.MESSAGE_REPLIED, userId: req.user.id }).sort({ updatedAt: -1 }).lean().exec();
...
messageRouter.post('/api/messsages/replies', async (req, res) => {
    ...
    if (req.body.type === 'featured') {
        ...
        await Profile.findOneAndUpdate({ _id: req.user.profileId }, { featured: value });
    }
    await Events.emit(EventTypes.MESSAGE_REPLIED, req.user.id, { messageType: req.body.type, response: req.body.value });
