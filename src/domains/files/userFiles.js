...
export const publicFile = (data) => { return { ... _.omit(data, ['_id', 'createdAt', 'updatedAt', '__v']), id: data['_id'] } };
...
export const putFile = async(file, Body, altName) => {
    ...
    await UserFile.findOneAndUpdate({ _id: file._id }, { $addToSet: { imageSizes: altName } });
    return true;
};
...
export const getFilesByIds = async (ids, omit = []) => {
    ...
    ids = ids.filter(d => d);
    const files = await UserFile.find({ _id: { $in: ids } }).lean().exec();
    return _.keyBy(files.map((f) => { return { ... _.omit(f, ['_id', '__v', ...omit]), id: f['_id'] } }), 'id');
}
...
fileRouter.post('/api/files/getfiles', async (req, res) => {
    ...
    const files = await UserFile.find({ _id: { $in: req.body.files } }).lean().exec();
    res.send({ status: 'ok', data: { files } });
});

fileRouter.post('/api/files/:filename', async (req, res) => {
    if ((_.get(req, 'body.category') !== 'verification') && ((!req.session || !req.session.userId))) {
        return res.status(400).send({ error: 'logged in user required' });
    }
    if ((!req.params.filename) || (!req.body) || (!req.body.category)) {
        return res.status(400).send({ error: 'filename, and category is required' });
    }
    if (['..', '/', '\\'].filter((c) => req.params.filename.indexOf(c) !== -1).length > 0) {
        return res.status(400).send({ error: 'invalid filename' });
    }
    const fid = generateSnowflake();
    const f = await UserFile.create({ _id: fid, userId: req.session.userId, parentId: req.body.parentId, category: req.body.category, filename: req.params.filename });
...
export const deleteFiles = async(fids) => {
    ...
    await Promise.allSettled(files.map(async (f) => {
        const keys = [ getBucketFilename(f, 'original') ]
            .concat(f.imageSizes.map((altName) => getBucketFilename(f, altName) ));
        console.log('deleting s3 objects', keys);
        const res = await s3.deleteObjects({
            Bucket, 
            Delete: { Objects: keys.map((Key) => { return { Key } }), },
        }).promise();
    }));
