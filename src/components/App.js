...
const AppRouter = ({ user }) => {
  ...
  const appClass = useMemo(
    () =>
      (urlparts.length > 1 && urlparts[1] === "media") ||
      url === "/moderator/images"
        ? "app assets"
        : "app",
    [url]
  );
      ...
      <div className={appClass}>
        ...
        <ErrorBoundary onError={onError}>
          <Router url={url}>
            ...
            {(hasRole(user, "admin") || hasRole(user, "mod")) && (
              <Moderator path="/moderator" />
            )}
            {(hasRole(user, "admin") || hasRole(user, "mod")) && (
              <Moderator path="/moderator/:section" />
            )}
            {(hasRole(user, "admin") || hasRole(user, "mod")) && (
              <Moderator path="/moderator/:section/:id" />
            )}
            <About path="/about" />
            <Fallback default />
          </Router>
...
const App = ({ url }) => {
  ...
  const currentProfile = profileStore.useState(
    (s) => s.profiles[user.profileId]
  );
        ...
        meta={[
          {
            name: "description",
            content:
              "Find new VR lewdies, and make your sexual fantasies come true in virtual reality 😊",
          },
          {
            name: "keywords",
            content:
              "vrchat, chilloutvr, erp, lewd, erp events, virtual reality, erotic role play",
          },
          {
            property: "og:title",
            content:
              "vrerp.net: Find partners for VR ERP, keep in touch with your lewdies",
          },
          {
