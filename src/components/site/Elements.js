...
export const Button = ({ children, color, ...rest }) => {
  return (
    <DraculaButton p="sm" color={color || "purple"} {...rest}>
      {children}
    </DraculaButton>
  );
};
...
export const outlinedButton =
  "drac-btn drac-bg-purple drac-btn-outline drac-text-purple";
...
export const SuspenseBar = ({ height, finished, load, children }) => {
  ...
  return loading ? (
    <div style={{ height, minHeight: "400px" }} className="skeleton-main"></div>
  ) : (
    children
  );
};
...
export const DynamicLink = ({ onClick, children, className = "" }) => {
  return (
    <div
      className={`dynamiclink drac-text-purple ${className}`}
      onClick={onClick}
    >
      {children}
    </div>
...
export const InputArea = ({ value, className, ...rest }) => {
  return (
    <div>
      <textarea
        className={`drac-input drac-input-purple drac-text-purple inputarea ${
          className ? className : ""
        }`}
        {...rest}
      >
  ...
  const checkClose = useCallback(
    (e) => {
      ...
      if (
        ...
        backgroundRef.current === e.target
      ) {
        onClose();
      }
    },
