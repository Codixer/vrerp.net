...
const uploadFile = async (file, body) => {
  const response = await fetch(`/api/files/${file.name}`, {
    method: "POST",
    body: JSON.stringify({ type: file.type, size: file.size, ...body }),
  });
...
const FileUpload = ({ onUpload, className, children, category, parentId }) => {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    const res = await Promise.all(
      acceptedFiles.map((f) =>
        uploadFile(f, { category: category || "profile", parentId })
      )
    );
    await onUpload(res);
    setUploading(false);
  ...
  return (
    <div className="drac-badge drac-bg-purple drac-badge-outline drac-text-purple drac-m-sm imageFrame">
      <div {...getRootProps()} className={classNames(className, "fileUpload")}>
        <input {...getInputProps()} />
        {isDragActive ? (
          <p className="fileUpload">Drop the files here ...</p>
        ) : uploading ? (
          <p className="fileUpload">Uploading ...</p>
        ) : (
          children
        )}
      </div>
