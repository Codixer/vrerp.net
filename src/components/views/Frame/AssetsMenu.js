  ...
  return (
    <div>
      ...
      <div className="sidemenu-item">
        <li>
          <Link href={`/${freePrefix}vrchat-quest-avatar-assets`}>
            Quest only
          </Link>
        </li>
      </div>
      <MenuLink href={`/${freePrefix}vrchat-accessory-assets`}>
        Accessories
      </MenuLink>
      <br />
      <Text p="sm" size="md">
      ...
      <br />
      <br />
      <DynamicMenuLink onClick={() => setShowPublish(true)}>
        ⚓ Publish items
      </DynamicMenuLink>
      {showPublish && (
        <Modal show={showPublish} onClose={() => setShowPublish(false)}>
          <AssetPublish />
        </Modal>
      )}
    </div>
