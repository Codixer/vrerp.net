...
export const MenuLink = ({ href, children }) => (
    <div className='sidemenu-item' key={ `menu${ href }` }><Link href={ href }>{ children }</Link></div>
);

export const DynamicMenuLink = ({ key, children, onClick }) => (
    <div className='sidemenu-item' key={ `menu${ key }` }><DynamicLink className='nodecoration' onClick={ onClick }>{ children }</DynamicLink></div>
)
...
export const MainMenu = () => {
    ...
    const setStatus = (evt) => fetchData(siteStore, `/api/users/status`, { status: evt.target.value });
    return (
        <div className='sidemenu-bar'>
            {
                ...
                <div className='sidemenu-menu'>
                    ...
                    <div className='sidemenu-item'>
                        <li><Link href='/profile'>edit bio</Link></li>
                    ...
                    {
                        (roles && roles.includes('dates')) && <MenuLink href="/dates">💝 Dates</MenuLink>
                    }
                    ...
                    {
                        (roles && roles.includes('events')) && <MenuLink href="/events">📅 Events</MenuLink>
                    }
                </div>
            }
            {
                hasRole(user, 'admin') && <div className='sidemenu-admin'>
                    <MenuLink href="/admin">🔔 Admin</MenuLink>
                </div>
            }            
            {
                hasRole(user, 'mod') && <div className='sidemenu-admin'>
