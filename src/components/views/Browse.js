...
const Browse = ({ username }) => {
  ...
  const navigateNext = useCallback(
    async (prevUsernameUrl) => {
      if (prevUsernameUrl) {
        profileStore.update((s) => {
          s.browseHistory.push(prevUsernameUrl);
        });
      }
      const res = await fetchStore(`/api/match/next`, null, [
    ...
    <div>
      {moreMatch && username && (
        <BrowseByURL
          ...
          onNext={navigateNext}
          onPrev={browseHistory.length === 0 ? null : navigatePrev}
        />
      )}
      {!moreMatch && (
        <div className="drac-m-sm">
          ...
          <Text>
            Check out the <Link href="/lobby">lobby</Link>, flirt on&nbsp;
            <a
...
const BrowseByURL = ({ usernameUrl, onNext, onPrev }) => {
  ...
  useEffect(async () => {
    if (!knownPages.includes(usernameUrl)) {
      await fetchData(
        siteStore,
        `/api/pages/${usernameUrl}`,
        null,
        (s, data) => (s.pages[usernameUrl] = data.page)
      );
    }
  }, [usernameUrl, page]);
  return (
