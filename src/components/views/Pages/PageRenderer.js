...
const ElementRenderer = ({ tag, attrs, children, ...props }) => {
  ...
  if (tag === "img" && props.imageId) {
    return (
      <Image
        file={{ category: "guide", extension: "jpg", id: props.imageId }}
        size="banner"
      />
    );
  }
...
const PageRenderer = ({ content, description, title, subtitle }) => {
  return (
    <div className="static-page">
      ...
      />
      <Link href="/">vrerp.net</Link>&nbsp;&nbsp;&raquo;&nbsp;&nbsp;
      <Link href="/guides">Guides</Link>
      <h1>{title}</h1>
      <Text size="sm">{subtitle}</Text>
      {content &&
        content.map((d, index) => <ElementRenderer key={index} {...d} />)}
    </div>
  );
};
