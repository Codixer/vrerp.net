...
const Guides = ({ url }) => {
  ...
  const knownPages = siteStore.useState((s) =>
    Object.values(s.pages).filter((p) => p.type === "page")
  );
  useEffect(async () => {
    await fetchStore(`/api/pages`, null, [
      [siteStore, (s, data) => (s.pages = { ...s.pages, ...data })],
    ]);
  }, [url]);
      ...
      {knownPages.map((page) => (
        <div key={page.url}>
          <h3>
            ...
            <br />
            <Text size="sm" className="graywhite">
              {page.subtitle}
            </Text>
            <br />
          </h3>
