...
const Report = ({ url, username }) => {
  ...
  return (
    <div>
      <Helmet
        title={`Report misbehavior - vrerp.net`}
        meta={[
          {
            name: "description",
            content: "Report misbehavior in-game, or the vr erp discord",
          },
      ...
      {!username && (
        <InputBox value={user} onChange={(evt) => setUser(evt.target.value)} />
      )}
      <br />
      Reason for report:
      <br />
      <h3>⚒️ Major violations:</h3>
      {REPORT_TAGS.major.map(({ name, value }) => (
        <ReportBox
          key={name}
          name={name}
          checked={tags.includes(name)}
          onToggle={(evt) => toggleTag(evt, name)}
        >
          {value}
        </ReportBox>
      ))}
      ...
      {REPORT_TAGS.redflag.map(({ name, value }) => (
        <ReportBox
          key={name}
          name={name}
          checked={tags.includes(name)}
      ...
      {REPORT_TAGS.yellowdot.map(({ name, value }) => (
        <ReportBox
          key={name}
          name={name}
          checked={tags.includes(name)}
      ...
      {tags.length > 0 && (
        <div>
          Please provide details, and screenshot if possible:
          <br />
          <InputArea
            name="details"
