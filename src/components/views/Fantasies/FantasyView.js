...
export const FantasyView = ({ id }) => {
  ...
  const profile =
    profileStore.useState((s) => s.profiles[profileId], [profileId]) || {};
  const fantasy = profileStore.useState((s) => s.fantasies[id], [id]) || {};
  const hasLoved =
    profile.fantasies &&
    profile.fantasies.love &&
    profile.fantasies.love.includes(id);
  const hasTrial =
    profile.fantasies &&
    profile.fantasies.love &&
    profile.fantasies.trial.includes(id);
  const files = profileStore.useState((s) => s.files);
  const image = useMemo(() => files[fantasy.image], [fantasy]);
  const fantasyProfileSet = (type) =>
    fetchData(
      profileStore,
      `/api/fantasy-profile/${id}/${type}`,
      {},
      updateProfile(profileId)
    );
  const fantasyProfileUnset = (type) =>
    deleteDataAndUpdateStore(
      profileStore,
      `/api/fantasy-profile/${id}/${type}`,
      updateProfile(profileId)
  ...
  return (
    <div className="static-page static-page-linebreaks">
      {
        <div className="float-right">
          <Link href={`/fantasies/${id}`} className="fantasy-link">
            🔗 link
          </Link>
          <br />
          {onboarded && !hasLoved && (
            <div>
              <Button
                onClick={() => fantasyProfileSet("love")}
                className="medium-size drac-btn drac-bg-purple drac-btn-outline drac-text-purple drac-p-sm"
              >
                💕 love
              </Button>
              <br />
          ...
          {onboarded && hasLoved && (
            <div>
              <Button
                onClick={() => fantasyProfileUnset("love")}
                className="medium-size drac-btn drac-bg-purple drac-btn-outline drac-text-purple drac-p-sm"
              >
                unlove
              </Button>
          ...
          {onboarded && !hasTrial && (
            <div>
              <Button
                onClick={() => fantasyProfileSet("trial")}
                className="medium-size drac-btn drac-bg-purple drac-btn-outline drac-text-purple drac-p-sm"
              >
                💙 wanna try
              </Button>
          ...
          {onboarded && hasTrial && (
            <div>
              <Button
                onClick={() => fantasyProfileUnset("trial")}
                className="medium-size drac-btn drac-bg-purple drac-btn-outline drac-text-purple drac-p-sm"
              >
                remove try
              </Button>
      ...
      {image && (
        <Image file={image} size="thumbnail" className="fantasy-image" />
      )}
      <br />
      {fantasy.text}
      <br />
      {fantasy.link && (
        <a href={fantasy.link} target="_blank" rel="noreferrer">
          source
        </a>
      )}
      <br />
    ...
    <SuspenseBar
      ...
      load={async () =>
        fetchData(
          ...
          `/api/fantasies/${id}`,
          null,
          updateFantasyData(id)
        )
      }
    >
      {fantasy.author && (
        <div>
          Posted by: <Link href={`/${fantasy.author}`}>{fantasy.author}</Link>
          <br />
          <br />
        </div>
      )}
      {fantasyLikes && fantasyLikes.length > 0 && (
        <div>
          💕 liked by:{" "}
          {fantasyLikes &&
            fantasyLikes.map((url, index) => (
              <span key={index}>
...
export const FantasyViewPage = ({ id }) => {
  ...
  return (
    <div>
      ...
      {profileId && (
        <div>
          <Link href="/fantasies">&laquo; fantasies</Link>
          <br />
          <br />
        </div>
      )}
