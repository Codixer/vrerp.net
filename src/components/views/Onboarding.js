...
const Onboarding = ({ section, url }) => {
    ...
    const onUpload = useCallback(async(fid) => {
        await fetchData(profileStore, `/api/profiles/${ user.profileId }/files`, { id: fid }, (s, data) => {
    ...
    return (<div>
        <Helmet title={ `Welcome to vrerp.net - find new lewdies!` } />
        {
            ...
            && <div>
                ...
                <Rules /><br /><br />
                <RulesUpdates /><br /><br />
                <Checkbox id='agreeRules' color='purple' checked={ rulesAccept } onClick={ () => setRulesAccept(!rulesAccept) } />
                <label htmlFor='agreeRules' className='drac-text'> I accept the rules, and am over 18 years old</label><br /><br />
                <center><Button p="sm" onclick={ () => route('/onboarding/basicinfo')  } disabled={ !rulesAccept } >Next &raquo;</Button></center>
            </div>
        }
        {
            section === 'basicinfo' 
            && <div>
                ...
                {
                    (!(profile.avatar && profile.username && profile.url && profile.vrchat)) && <div>Upload a pic of your avatar, and set your username, and vrchat to continue!</div>
                }
                <center><Button onclick={ () => route('/onboarding/tags')  } 
                    disabled={ !(profile.avatar && profile.username && profile.url && profile.vrchat) }
                >Next &raquo;</Button></center>
            </div>
        }
        {
            section === 'tags'
            && <div>
                ...
                <center><Button onclick={ () => { syncTags(); route('/onboarding/bio'); } } >Next &raquo;</Button></center>
            </div>
        }
        {
            section === 'bio'
            && <div>
                ...
                <br />
                {
                    (!profile.onlineTimes || (!profile.lookingFor || profile.lookingFor.length < 10) || (!profile.bio || profile.bio.length < 10)) && <div>
                        Please write a bit about yourself!<br />
                        All fields are required.
                    </div>
                }
                <br />
                <center><Button onclick={ () => route('/onboarding/verification') }
                    disabled={ !profile.onlineTimes || (!profile.lookingFor || profile.lookingFor.length < 10) || (!profile.bio || profile.bio.length < 10) }
                >Next &raquo;</Button></center>
            </div>
        }
        {
            section === 'verification'
            && <UserVerification userId={ user.id } onFinished={ () => route('/onboarding/settings') } />
        }
        {
            section === 'settings'
            && <div>
                ...
                <center><Button onclick={ () => route('/onboarding/profile')  }>Next &raquo;</Button></center>
            </div>
        }
        {
            section === 'profile'
            && <div>
                <ProfileView id={ user.profileId } /><br />
                {
                    <FileUpload className={ `` } onUpload={ onUpload } parentId={ user.profileId } >
                        {
                            <center className="fileUpload">+ upload more images</center>
                        }
                    </FileUpload>
                }
                <center><Button onclick={ () => publish()  }>Finish &raquo;</Button></center>
            </div>
        }
    </div>);
};
