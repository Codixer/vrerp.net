...
import {
  AdminProfileSetStatus,
  AdminProfileThumbView,
  AdminUserSearch,
  AdminVerification,
  ...
  return (
    <div>
      {listData.map(({ source, target, matchDate }, index) => {
        ...
        return (
          <ProfileViewThumbnail key={`${index}-${source}`} id={source}>
            <b>
              <Link href={`/${profiles[source].url}`}>
                {profiles[source].username}
              </Link>
            </b>
            &nbsp;&nbsp;💕&nbsp;&nbsp;
            <b>
              <Link href={`/${profiles[target].url}`}>
...
const AdminTagsEdit = () => {
  ...
  const addEventTag = useCallback(async () => {
    await fetchData(calendarStore, "/api/admin/calendartags", {
      name: newEventTag,
    });
  });
    ...
    <div>
      ...
      >
        ...
        <InputBox
          name="kink"
          value=""
          onBlur={(evt) => setNewKink(evt.target.value)}
        >
          New kink:{" "}
        </InputBox>
        <Button onClick={addKink}>Add kink</Button>
        {get(schema, "kinks.values") &&
          schema.kinks.values.map((k, index) => <div key={index}>{k}</div>)}
      </SuspenseBar>
      <br />
      <SuspenseBar
        height="30vh"
        ...
        load={async () =>
          (!calendarTags || calendarTags.length === 0) &&
          fetchData(calendarStore, `/api/calendar/schema`)
        }
      >
        <b>Event tags:</b>
        <InputBox
          name="eventtag"
          value=""
          onBlur={(evt) => setNewEventTag(evt.target.value)}
        >
          New event tag:{" "}
        </InputBox>
        <Button onClick={addEventTag}>Add event tag</Button>
      ...
      <SuspenseBar
        ...
        load={async () =>
          ...
          fetchData(assetStore, `/api/assets/schema`)
        }
      >
        <b>Asset tags:</b>
        <InputBox
          name="assettag"
          value=""
          onBlur={(evt) => setNewAssetTag(evt.target.value)}
        >
          New asset tag:{" "}
        </InputBox>
        <Button onClick={addAssetTag}>Add asset tag</Button>
...
const AdminGuides = () => {
  ...
  return (
    <div>
      ...
      <br />
      <a
        href="https://www.google.com/ping?sitemap=https://vrerp.net/sitemap.xml"
        target="_blank"
        rel="noreferrer"
      >
...
const AdminAssets = () => {
  ...
  return (
    <div>
      <InputBox
        name="title"
        onBlur={(evt) => setNewAsset(evt.target.value)}
        className="w240"
        placeholder="https://"
      >
        Add new asset:{" "}
      ...
      <DataList
        loader={(lastId) =>
          fetchStore(
            addParameters("/api/admin/assets?limit=32", { lastId }),
            null,
            [
  ...
  <div>
    ...
    {section === undefined && (
      <div>
        ...
        <br />
        <li>
          <Link href="/moderator/profiles">list profiles scroller</Link>
        </li>
        <li>
          <Link href="/admin/tags">edit kinks &amp; tags</Link>
        </li>
        <li>
          <Link href="/moderator/verification">id verification</Link>
        </li>
        <li>
          <Link href="/moderator/verification-audit-log">
