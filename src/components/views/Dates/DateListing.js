...
const DateEntry = ({ date }) => {
  ...
  const profile = useMemo(
    () => getProfileByUserId(profiles, otherUserId),
  ...
  return (
    <div className="profile-view-thumbnail">
      ...
      <div className="profile-view-thumbnail-info">
        <Link href={`/${profile.url}`}>{profile.username}</Link>&nbsp; invited
        you to a date on {formatDate(date.startDate)} at{" "}
        {formatTime(date.startDate)}
        <br />
        <br />
        <Button
          onClick={() => respondDate("accept")}
  ...
  return (
    <div>
      <SuspenseBar
        ...
        load={() =>
          fetchStore(`/api/dates`, null, [
            [profileStore, updateProfile()],
            [assetStore, updateAssets],
            [datesStore, updateDates],
          ])
        }
      >
        <h2>Invites</h2>
        {dates &&
          dates.length > 0 &&
          dates
            .filter((d) => d.status === "invited" && d.inviter !== userId)
            .map((date) => <DateEntry key={date.id} date={date} />)}
