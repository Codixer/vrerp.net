  ...
  const isSelecting = useCallback(
    (x, y) => {
      ...
      if (
        ...
        (selection[3] > x || selection[1] < x)
      ) {
        return false;
      }
      if (
        selection[1] <= selection[3] &&
        (selection[1] > x || selection[3] < x)
      ) {
        return false;
      }
      if (
        selection[2] >= selection[4] &&
        (selection[4] > y || selection[2] < y)
      ) {
        return false;
      }
      if (
        selection[2] <= selection[4] &&
        (selection[2] > y || selection[4] < y)
      ) {
        return false;
      }
      return true;
  ...
  return (
    <div>
      ...
      <SuspenseBar
        ...
        load={async () =>
          fetchData(datesStore, "/api/dates/available").then((data) =>
            init(data)
          )
        }
      >
        <h1>💝 Dates</h1>
        <b>When are you available for erp dates? 😊</b>
        <br />
        <TimeZoneSelect />
        When are you usually available?
        <br />
        <small>Desktop: Click and drag to toggle; Mobile: tap to toggle</small>
        <br />
        <table border="1">
          <thead>
            {[" "].concat(dayHeaders).map((day) => (
              <th key={day}>{day}</th>
            ))}
          </thead>
          <tbody>
