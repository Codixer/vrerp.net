...
const DateProfile = ({ url, username }) => {
  ...
  const urlprofiles = Object.values(knonwProfiles).filter(
    (p) => p.url === username
  );
  ...
  const sendInvite = useCallback(async () => {
    setLoading(true);
    await fetchStore(`/api/dates/invite`, {
      startDate,
      message,
      invited: profile.userId,
    });
    route("/dates");
  });
  console.log({
  ...
  return (
    <div>
      <SuspenseBar
        ...
        load={async () =>
          Promise.all([
            fetchStore(`/api/dates/available/${username}`, null, [
            ...
            timezones.length === 0 &&
              fetchData(datesStore, "/api/dates/available"),
          ])
        }
      >
        <h1>Invite {profile && profile.username} to a date! 💕</h1>
        {avatar && (
          <Image file={avatar} size={"thumbnail"} className="float-right" />
        )}
          ...
          <div>
            ...
            {availableTimes.map((item, index) => {
              ...
              if (index > 0) {
                ...
                prev.setMinutes(
                  prev.getMinutes() + availableTimes[index - 1] * 30
                );
        ...
        {selectedTime && (
          <div>
            ...
            />
            <br />
            <Button className={outlinedButton} onClick={sendInvite}>
