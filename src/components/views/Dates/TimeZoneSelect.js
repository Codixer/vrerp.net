...
const TimeZoneSelect = ({ onChange }) => {
  ...
  const init = useCallback((data) => {
    ...
    if (data.userTimezone === null) {
      ...
      for (let i = 0; i < data.timezones.length; i++) {
        if (data.timezones[i].currentTimeOffsetInMinutes === offset) {
          ...
          fetchPost("/api/dates/timezones", {
            userTimezone: data.timezones[i].name,
            userTimezoneOffset: data.timezones[i].rawOffsetInMinutes,
          });
          break;
        }
      }
    ...
    datesStore.update((s) => {
      s.timezones = data.timezones;
      s.userTimezone = data.userTimezone || detectedTz;
    });
  }, []);
  const changeHandler = useCallback(
    async (evt) => {
      datesStore.update((s) => {
        s.userTimezone = evt.target.value;
      });
      const userTimezoneOffset = timezones.find(
        (tz) => tz.name === evt.target.value
      ).rawOffsetInMinutes;
      await fetchPost("/api/dates/timezones", {
        userTimezone: evt.target.value,
        userTimezoneOffset,
      });
      if (onChange) {
  ...
  return (
    <div>
      <SuspenseBar
        ...
        load={async () =>
          fetchData(datesStore, "/api/dates/timezones").then((data) =>
            init(data)
          )
        }
      >
          ...
          {timezones &&
            timezones.map((tz) => (
              <option
                key={tz.name}
                value={tz.name}
                timezoneOffset={tz.rawOffsetInMinutes}
                selected={true}
              >
                {tz.name}
              </option>
