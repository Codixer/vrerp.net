...
const PrivacySettings = ({ id }) => {
  ...
  const handleSwitch = useCallback(
    (key, value) => fetchProfile(id, profileStore, { [key]: value }),
    [profileStore, id]
  );
  return (
    <div>
      ...
      <br />
      Public means visible without logging in (but not to search engines). This
      allows you to direct link your profile (vrerp.net/{profile.username}) in
      lewd discord servers for detailed bio and availability 😊
      <br />
      <br />
      Discord &amp; vrchat username visible to:
      <Select
        color="purple"
        onChange={handleChange("discordVisibility")}
        defaultValue={profile.discordVisibility}
      >
        <option value="public">public</option>
        <option value="members">members only</option>
      ...
      <Select
        color="purple"
        onChange={handleChange("fantasiesVisibility")}
        defaultValue={profile.fantasiesVisibility}
      >
        <option value="public">public</option>
        <option value="members">members only</option>
        <option value="matches">matches only</option>
      ...
      {roles && roles.includes("dates") && (
        <div>
          ...
          <Select
            color="purple"
            onChange={handleChange("datesVisibility")}
            defaultValue={profile.datesVisibility}
          >
      ...
      {hasRole(user, "featured") && (
        <Text p="md" size="md">
          ...
          />
          <label htmlFor="featured" className="drac-text">
