...
const NotificationSettings = ({ id }) => {
    ...
    const switchNotifications = useCallback(async (bitValue) => {
        setLoading(true);
        let result = (discordNotifications & bitValue) ? (discordNotifications ^ bitValue) : (discordNotifications | bitValue);
        ...
        <div>
            ...
            <Text p="md" size="md">
                <Switch id='onInvite' color='purple' 
                    ...
                    disabled={ loading }
                />
                <label htmlFor='onInvite' className='drac-text'> On invites</label>
            </Text><br /><br />
            <Text p="md" size="md">
                <Switch id='onMatch' color='purple' 
                    checked={ !!(discordNotifications & NOTIFICATION_MATCH) } 
                    onClick={ () => switchNotifications(NOTIFICATION_MATCH) } 
                    disabled={ loading }
                />
                <label htmlFor='onMatch' className='drac-text'> On matching</label>
            </Text><br />
            <br />
            In order to get Discord notifications, you need to <a href={ 'https://discord.gg/HDZQP6Wb6f' } target='_blank' rel='noreferrer'>join our discord server</a>.
            <br />
        </div>
    )
};
