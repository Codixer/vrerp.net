...
export const Invites = () => {
  ...
  const shownInvites = useMemo(
    () =>
      invites.filter(
        (profileId) =>
          profiles[profileId] &&
          profiles[profileId].profileVisibility !== "disabled"
      ),
    [invites, profiles]
  );
  const missingProfiles = useMemo(
    () => invites.filter((profileId) => !profileList.includes(profileId)),
    [invites, profileList]
  );
  ...
  return (
    ...
    >
      ...
      {missingProfiles.length === 0 &&
        shownInvites.map((id) => (
          <ProfileViewThumbnail key={id} id={id}>
            {profiles[id].username} wants to be your lewdie 💕
            <br />
            <Button
      ...
      {missingProfiles.length === 0 && shownInvites.length === 0 && (
        <div className="drac-m-sm">
          No invites for now. <Link href="/browse">Make some friends!</Link>
          <br />
        </div>
      )}
      <br />
