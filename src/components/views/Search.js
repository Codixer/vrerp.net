...
const Search = () => {    
    ...
    //     }
    // }, [ route ]);
    const schemaTags = useMemo(() => Object.values(schema).reduce((arr, val) => arr.concat(
        val.values.map((val) => typeof val === 'string' ? val : val.display)
    ).filter(t => !['female'].includes(t.toLowerCase())), []), [ schema ]);
    return <div>
        <div className='float-right'>Sort by:<br />
        ...
        <MultipleComboBox items={ schemaTags } >
            {
                ({ selectedItems }) => {
                    return <div>Results: { selectedItems.join(' ') }
                        <ProfileList apiurl={ `/api/search?q=${ selectedItems.map(s => encodeURIComponent(s.replace(/-/g, '_').split(' ').join('-'))).join('+') }` +
                            (sort ? `&sort=${ sort }` : '') } />
                    </div>
                }
            }
