        ...
        <table>
            {
                Object.keys(moods).map(id => {
                    ...
                    let user = users[moods[id][0].userId] || { username: `unknown ${ moods[id][0].userId }`, url: 'unknown' }
                    const replies = moods[id].reduce((arr, val) => { arr[val.parameters.messageType] = val.parameters.response; return arr; }, { });
                    console.log(replies);
                    return (<tr key={ id }>
                        <td>{ moods[id][0].createdAt.substr(0,10) }</td>
                        <td><Link href={ `/${ user.url }` } >{ user.username }</Link></td>
                        <td>
