...
export const AdminProfileThumbView = ({ profile }) => (
  <span>
    <span
      ...
      id={`profile-status-display-${profile.id}`}
      key={`profile-status-display-${profile.id}`}
    >
    ...
    {formatDate(new Date(profile.lastActivity))}
    <div className="admincontrols">
      <Link href={`/moderator/profiles/${profile.id}`}>edit</Link>
    </div>
...
export const AdminUserSearch = () => {
  ...
  return (
    <div>
      <InputBox
        onKeyUp={(evt) => setQuery(evt.target.value)}
        className="w240"
        placeholder="search..."
      />
        ...
        <div key={p.id}>
          <div className="profile-view-thumbnail">
            ...
            <div className="profile-view-thumbnail-info">
              ...
              <Select
                color="purple"
                onChange={handleRejectionChanged(p.id)}
                defaultValue="none"
              >
                <option value="none">No reason</option>
                <option value="profilepic">Profile pic not avatar</option>
                <option value="discordmigration">
                  Not approved discord verif
                </option>
                <option value="discordpaper">
                  No discord on paper on verification
                </option>
              ...
              <br />
              <Button onClick={() => setStatus(p.id, "verified")}>
                Approve
              </Button>
...
export const AdminProfileSetRoles = ({ id }) => {
  ...
  const toggleRole = useCallback(
    async (evt) => {
      ...
      const newRoles = roles.includes(roleToggle)
        ? roles.filter((r) => r !== roleToggle)
        : roles.concat([roleToggle]);
...
export const AdminProfileList = ({ membersOnly = false }) => {
  ...
  return (
    <div>
      {!membersOnly && (
        <Link href="/moderator/member-profiles" className="float-right">
          members only
        </Link>
      )}
      {membersOnly && (
        <Link href="/moderator/profiles" className="float-right">
          all profiles
        </Link>
      )}
      ...
      <ProfileList
        apiurl={
          membersOnly
            ? "/api/moderator/member-profiles"
            : "/api/moderator/profiles"
        }
      >
        {({ profile }) => <AdminProfileThumbView profile={profile} />}
      </ProfileList>
...
export const AdminProfileSetStatus = ({ id }) => {
  ...
  const moderateUser = async (status) => {
    ...
    await fetchData(
      profileStore,
      `/api/moderator/moderate/${id}/${status}`,
      { adminMessage },
      updateProfile(id)
    );
    if (status === "delete") {
  ...
  return (
    <div>
      <br />
      User status: {profile.profileVisibility}
      <br />
      <Button onclick={() => setStatus(updateStatus)}>
        Set status to {updateStatus} &raquo;
      </Button>
      <br />
      <br />
      <Button onclick={() => moderateUser("suspended")}>Suspend user</Button>
      <br />
      <br />
      <Button onclick={() => moderateUser("revoke")}>
        Revoke verification
      </Button>
      <br />
      <br />
      <Button onclick={() => moderateUser("banned")}>Ban user</Button>
      <br />
      <br />
      <Button onclick={() => moderateUser("delete")}>Delete user</Button>
      <br />
    </div>
  );
};
