...
const Fallback = ({ url, header }) => {
    ...
    return (
        <SuspenseBar height='30vh' 
            ...
            load={ async() => fetchData(siteStore, `/api/pages/${ url.substr(1) }`, null, (s, data) => s.pages[url.substr(1)] = data.page) }
        >
            {
                header && <Header />
            }
            ...
            }
            {
                (page && page.type === 'profile' && knonwProfiles[page.id] !== null)
                    && <ProfileView id={ page.id } controls={ user.profileId && hasRole(user, 'onboarded') && user.profileId !== page.id } />
            }
            {
                (page && page.type === 'profile' && knonwProfiles[page.id] === null)
                    && <div className='drac-m-sm'>
                        <Helmet 
                            ...
                            meta={[
                                ...
                                { property: 'og:title', content: `${ page.username } on vrerp.net` },
                                { property: 'og:image', content: page.image },
                                { property: 'og:description', content: page.description }                    
                            ]}
                        />

                    This profile is not public -you need to be a member to see it!<br /><br />
                        <LoginFields redirectUrl={ url } />
                    </div>
            }
            {
                (page && page.type === 'notfound') && <div className='drac-m-sm'>
