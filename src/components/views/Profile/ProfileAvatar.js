...
const ProfileAvatar = ({ id, edit, size }) => {
    ...
    if (edit === true) {
        return (
            <FileUpload className={ `avatar avatarUpload` } onUpload={ onUpload } parentId={ user.profileId } >
                {
                    profile.avatar && <Image file={ avatar } size='thumbnail' className='avatarMainimage' alt={ profile.username } />
                }
                {
                    !profile.avatar && <center className="fileUpload">No avatar, click here to upload one</center>
                }
            </FileUpload>
        )
    }
    return (
        <div className='avatar'>
            {
                profile.avatar && <Image file={ avatar } size={ size || 'thumbnail' } className='mainimage' alt={ profile.username } />
            }
            {
