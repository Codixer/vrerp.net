...
export const ProfileControls = ({ profile }) => {
  ...
  const openDiscord = useCallback(
    (id) => window.open(`https://discord.com/users/${id}`),
    [profile]
  );
  ...
  return (
    <div>
      <div>
        {!isMatch && !isInvitePending && (
          <Button
            onClick={() => handleMatch(profile.id, "love")}
            variant="outline"
        ...
        {!isMatch && isInvitePending && (
          <Button
            onClick={() => handleMatch(profile.id, "pass")}
            variant="outline"
            className="medium-size drac-btn drac-bg-purple drac-btn-outline drac-text-purple drac-p-sm"
          >
            Cancel invite
        ...
        {profile.discordId && (
          <div>
            <Button
              onClick={() => openDiscord(profile.discordId)}
              variant="outline"
              className="medium-size drac-btn drac-bg-purple drac-btn-outline drac-text-purple drac-p-sm"
            >
              💌 Message
        ...
        {isMatch && (
          <div>
            <Button
              onClick={() => setShowCompliments(true)}
              variant="outline"
              className="medium-size drac-btn drac-bg-purple drac-btn-outline drac-text-purple drac-p-sm"
            >
              🤗 Compliment
        ...
        {profile.allowDates && roles && roles.includes("dates") && (
          <div>
            <br />
            <Button
              onClick={() => route(`/dates/${profile.url}`)}
              variant="outline"
              className="medium-size drac-btn drac-bg-purple drac-btn-outline drac-text-purple drac-p-sm"
            >
              💝 Date
        ...
        <Button
          onClick={() => route(`/report/${profile.url}`)}
          variant="outline"
          className="medium-size drac-btn drac-bg-purple drac-btn-outline drac-text-purple drac-p-sm"
        >
          Report
      ...
      {showCompliments && (
        <Modal show={showCompliments} onClose={() => setShowCompliments(false)}>
          <Compliments
            profile={profile}
            onClose={() => setShowCompliments(false)}
          />
        </Modal>
      )}
    </div>
