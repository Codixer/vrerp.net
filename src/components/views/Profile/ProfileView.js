...
import { fetchData, fetchProfile, fetchStore, updateAssets, updateProfile } from '../../scripts/api.js';
...
const ProfileView = ({ id, controls, showMatchbox }) => {
    ...
    if ((profile.profileVisibility) && (profile.profileVisibility === 'banned')) {
        return <div>This user was banned.<br/><br/><Link href="/">Go back</Link></div>;
    }
    ...
    return (
        <SuspenseBar height='30vh' 
            ...
            load={ async() => Promise.all([
                fetchStore(`/api/profiles/${ id }`, null, [[ profileStore, updateProfile(id) ], [ assetStore, updateAssets ]]),
        ...
        >
            <Helmet 
                ...
                meta={[
                    ...
                    { property: 'og:image', content: getImageURL(files[profile.avatar], 'thumbnail') },
                    { property: 'og:description', content: getDescription(profile.bio) }                    
                ]}
            />
            {
                ((currentProfileId !== id) && (isInvited) && (showMatchbox !== false)) && <MatchBox id={ id } onNext={ () => {} } />
            }
            <ProfileAvatar id={ id } size="banner" />
            <span className='float-right' id={ `profile-status-display-${ id }` } key={ `profile-status-display-${ id }` }>
                <center><ProfileStatus status={ profile.status } /></center><br />
                { (controls === true) && <ProfileControls profile={ profile } /> }
                { ((controls !== true) && (profile.id !== user.profileId)) && <ProfileControlReport profile={ profile } /> }
            </span>
            <strong>{ profile.username }</strong><br />
            {
                profile.discord && <div><b>Discord: { profile.discord }</b><br /></div>
            }
            {
                profile.vrchat && <div><b>VRChat: { profile.vrchat }</b><br /></div>
            }
            {
                profile.chilloutvr && <div><b>ChilloutVR: { profile.chilloutvr }</b><br /></div>
            }
            {
                profile.link && <div><a href={ profile.link } target='_blank' rel='noreferrer'>{ profile.link }</a><br /></div>
            }
            <br />Bio:<br />
            <ProfileViewTags profile={ profile } />
            <br /><br />
            {
                profile.onlineTimes && <div><b>Usually online: </b>{ profile.onlineTimes }<br /></div>
            }
            <br />
            {
                profile.lookingFor && <div><b>Looking for: </b><div className='user-text'>{ profile.lookingFor }</div><br /></div>
            }
            <div style={{ clear: 'both' }}></div>
            <br />
            {
                profile.bio && <div><b>Bio: </b><div className='user-text'>{ profile.bio }</div><br /></div>
            }
            <br />
            ...
            {
                (profile.files && profile.files.length > 0) && <div>
                    <b>Media:</b><br />
                    <Gallery fileIds={ profile.files } />
                </div>
            }
            {
                (profile.fantasies && !profile.fantasies.trial && !profile.fantasies.love && profile.fantasies.count > 0) && <div>
                    <br />
                    { profile.fantasies.count } fantasies shown to { profile.fantasiesVisibility } only.&nbsp;
                    {
                        !currentProfileId && <span><Link href="/">Sign up to see more!</Link></span>
                    }
                </div>
            }
            {
                (profile.fantasies && profile.fantasies.trial && (profile.fantasies.trial.length > 0 || profile.fantasies.love.length > 0)) && <div>
                    <br /><br /><center><Heading size='xl'>✨ fantasies</Heading></center>
                </div>
            }
            {
                (profile.fantasies && profile.fantasies.love && profile.fantasies.love.length > 0) && <div>
                    <br /><br />
                    <Heading size='lg'>💕 love:</Heading>
                    {
                        profile.fantasies.love.map((id) => <div key={ `love-${ id }` }><FantasyView key={ `love-${ id }` } id={ id } /><br /><br /></div>)
                    }
                </div>
            }
            {
                (profile.fantasies && profile.fantasies.trial && profile.fantasies.trial.length > 0) && <div>
                    <br /><br />
                    <Heading size='lg'>💙 wanna try:</Heading>
                    {
                        profile.fantasies.trial.map((id) => <div key={ `trial-${ id }` }><FantasyView key={ `trial-${ id }` } id={ id } /><br /><br /></div>)
                    }
                </div>
            }
            {
                (profile.wishlist && profile.wishlist.length > 0) && <div>
                    <br /><br /><center><Heading size='lg'>🎁 Wishlist:</Heading></center><br />
                    {
                        profile.wishlist.map((id) => <AssetMasonryItem id={ id } key={ `asset-${ id }` } />)
                    }
                </div>
            }
            <br />
