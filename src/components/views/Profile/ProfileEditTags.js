...
const ProfileEditTags = ({ id }) => {
  ...
  const getKey = (value) =>
    !!value && value.constructor === Object ? value.display : value;
  const hasKey = (arr, value) =>
    typeof arr === "string" ? arr === value : arr.includes(value);
  const getSelected = (key, value) =>
    profile[key] && hasKey(profile[key], getKey(value));
  ...
  return (
    ...
    >
      ...
      {schemaOrder.map((key) => (
        <div key={key}>
          {schema[key].description ? schema[key].description : key}
          <br />
          <br />
          {schema[key].values.map((val) => (
            <TagBubble
              key={getKey(val)}
              selected={getSelected(key, val)}
              value={val}
              onClick={() => toggleSelected(key, getKey(val))}
            />
          ))}
          <br />
          <br />
