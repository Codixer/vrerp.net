...
export const get = (obj, key) =>
  (Array.isArray(key) ? key : key.split(".")).reduce(
    (val, key) => (val === undefined ? undefined : val[key]),
    obj
  );
...
export const siteUrl = clientSide
  ? `${window.location.protocol}//${window.location.host}/`
  : process.env.SERVER_URL;

export const getDescription = (s) =>
...
export const addParameters = (url, parameters) =>
  Object.keys(parameters).reduce((url, item) => {
    return parameters[item]
      ? `${url}${
          url.indexOf("?") === -1 ? "?" : "&"
        }${item}=${encodeURIComponent(parameters[item])}`
      : url;
  }, url);
