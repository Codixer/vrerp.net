...
import {
  ...
  getProfileData,
  getProfileList,
} from "../domains/users/profiles.js";
...
import {
  ...
  getFantasyLikes,
  getFantasyList,
} from "../domains/fantasies/fantasies.js";
...
const mergeDeep = (trg, src) => {
  Object.keys(src).map((key) => {
    trg[key] =
      trg[key] && isObject(trg[key]) && isObject(src[key])
        ? mergeDeep(trg[key], src[key])
        : src[key];
  });
  return trg;
};
...
const assetPages = [
  "vrchat-avatar-assets",
  "vrchat-quest-avatar-assets",
  "vrchat-accessory-assets",
  "vrchat-erp-assets",
];
...
router.get("/sitemap.xml", async (req, res) => {
  const lastDeployed = fs
    .statSync(process.argv[1])
    .mtime.toISOString()
    .substring(0, 10);
...
const getLayout = async (req, res) => {
  ...
  const { html, links } = await prerender(ssrApp, {
    renderOpts: { pretty: process.env.MODE === "development" },
  });
...
<body>
    ...
    posthog.init('phc_EjTzFtHvMitsUM3bbODI8bsGMxcyFnleymtqEFsccs4',{api_host:'https://eu.posthog.com'})
</script>

    <script>window.__STATE__=${JSON.stringify({ url }).replace(
      /<|>/g,
      ""
    )}</script>
    <script>window.__PULLSTATE__ = '${JSON.stringify(
      req.stores.getPullstateSnapshot()
    )
      .replace(/\\/g, `\\\\`)
      .replace(/"/g, `\\"`)
      .replace(/'/g, `\\'`)}'</script>
...
${
  process.env.MODE === "development"
    ? `<!-- ${JSON.stringify(
        req.stores.getPullstateSnapshot().allState,
        null,
        4
      )} -->`
    : ""
...
router.get("/fantasies", async (req, res, next) => {
  ...
  mergeData(
    req,
    "profileStore",
    JSON.parse(JSON.stringify(await getFantasyList()))
  );
  next();
});

router.get(
  ["/", "/:page", "/:page/:subpage", "/:page/:subpage/:subsubpage"],
  async (req, res, next) => {
    const url = parseurl(req).pathname;
    ...
    if (profileIds.length > 0) {
      const { profiles, files, fantasies, assets } = await getProfileList(
        req,
        profileIds
      );
      mergeData(
